...
1. файлы ["mosaic_hits*"](https://github.com/Vetselet/SHERE-2_from_appro_to_mass/tree/main/mosaic_hits_example). В верхней строчке обозначены **номер клона** события, **высота детектора** со знаком минус, координаты **х,у** оси ливня на снегу, **номер события**;
2. файл ["mosaic_hits_coord"](), который содержит данные координат центров ФЭУ;
3. файлы из папки ["BG_files"]() для учета фона;
4. файлы из папки ["aberration_data"]() для учета сферических аберраций.
